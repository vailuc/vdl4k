...
# Setup local bin directory using XDG specification
...
# Add to PATH if not already there (XDG compliant)
if ! echo "$PATH" | grep -q "$LOCAL_BIN"; then
    ...
    if [ -f ~/.zshrc ]; then
        # Remove any existing vdl4k PATH entries to avoid duplicates
        sed -i '/export PATH.*local.*bin.*PATH/d' ~/.zshrc 2>/dev/null || true
...
# If we're in ~/.local/bin (XDG standard), look for the project in standard locations
if [[ "$PROJECT_DIR" == *".local/bin" ]] && [ -d "$PROJECT_DIR/../vdl4k" ]; then
    PROJECT_DIR="$PROJECT_DIR/../vdl4k"
elif [[ "$PROJECT_DIR" == *".local/bin" ]] && [ -d "$HOME/vdl4k" ]; then
    PROJECT_DIR="$HOME/vdl4k"
elif [[ "$PROJECT_DIR" == *".local/bin" ]] && [ -d "$(dirname "$SCRIPT_DIR")/vdl4k" ]; then
    PROJECT_DIR="$(dirname "$SCRIPT_DIR")/vdl4k"
fi

# Fallback: search for vdl4k project directory in standard locations
if [[ "$PROJECT_DIR" == *".local/bin" ]]; then
    # First try to find in common locations
    for search_dir in "$HOME/Documents" "$HOME/PycharmProjects" "$HOME/Projects" "$HOME/Desktop" "$HOME/src" "$HOME/code"; do
...
if [[ "$PROJECT_DIR" == *".local/bin" ]] || [ ! -d "$PROJECT_DIR/lib" ]; then
    ...
    if [ -z "$PROJECT_DIR" ] || [ ! -f "$PROJECT_DIR/lib/config.sh" ]; then
        ...
        echo "Searched locations: /home/*/Documents, /home/*/PycharmProjects, /home/*/, ~/.local/*" >&2
        exit 1
    fi
fi
